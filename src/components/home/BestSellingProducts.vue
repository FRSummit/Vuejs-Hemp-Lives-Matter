<template>
  <div class="best-selling-products" id="best-selling-products">
...
export default {
  ...
  created() {
    ...
    firebase
      .database()
      .ref("hlm_home_best_selling_products")
      .on("value", snapshot => {
  ...
  methods: {
    ...
    removeItem(key) {
      firebase
        .database()
        .ref("hlm_home_best_selling_products/" + key)
        .remove();
    }
  }
};
...
/* Form / v-card */
#best-selling-products .v-sheet.v-card {
  width: 60%;
  margin: 0 auto;
  position: absolute;
  top: 60px;
